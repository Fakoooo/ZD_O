...
function script.update(dt)
    ...
    if player.engineLifeLeft < 1 then
        if totalScore > highestScore then
            ...
            ac.sendChatMessage("[ZD] scored " .. totalScore .. " points.")
        end
        totalScore = 0
        comboMeter = 1
        return
...
function event.addMessage(text, mood)
    for i = math.min(#event.messages + 1, 4), 2, -1 do
        event.messages[i] = event.messages[i - 1]
        event.messages[i].targetPos = i
    end
    event.messages[1] = {
        text = text,
        age = 0,
    ...
    if mood == 1 then
        for i = 1, 60 do
            ...
            event.glitterCount = event.glitterCount + 1
            event.glitter[event.glitterCount] = {
...
-- Function to update messages and glitter
function event.updateMessages(dt)
    event.comboColor = event.comboColor + dt * 10 * event.comboMeter
    if event.comboColor > 360 then
        event.comboColor = event.comboColor - 360
    end
    for i = 1, #event.messages do
        local m = event.messages[i]
        m.age = m.age + dt
        m.currentPos = math.applyLag(m.currentPos, m.targetPos, 0.8, dt)
    end
    for i = event.glitterCount, 1, -1 do
        local g = event.glitter[i]
        g.pos:add(g.velocity)
        g.velocity.y = g.velocity.y + 0.02
        g.life = g.life - dt
        g.color.mult = math.saturate(g.life * 4)
        if g.life < 0 then
            if i < event.glitterCount then
                event.glitter[i] = event.glitter[event.glitterCount]
            end
            event.glitterCount = event.glitterCount - 1
        end
    end
    if event.comboMeter > 10 and math.random() > 0.98 then
        for i = 1, math.floor(event.comboMeter) do
            local dir = vec2(math.random() - 0.5, math.random() - 0.5)
            event.glitterCount = event.glitterCount + 1
            event.glitter[event.glitterCount] = {
                color = rgbm.new(hsv(math.random() * 360, 1, 1):rgb(), 1),
...
function event.drawUI()
    local uiState = ac.getUiState()
    event.updateMessages(uiState.dt)
    ...
    local colorCombo =
        rgbm.new(hsv(event.comboColor, math.saturate(event.comboMeter / 10), 1):rgb(), math.saturate(event.comboMeter / 4))
    ...
    ui.text("[ZD] Highest Score: " .. event.highestScore .. " pts")
    ui.popFont()
    ui.popStyleVar()

    ui.pushFont(ui.Font.Title)
    ui.text(event.totalScore .. " pts")
    ui.sameLine(0, 20)
    ui.beginRotation()
    ui.textColored(math.ceil(event.comboMeter * 10) / 10 .. "x", colorCombo)
    if event.comboMeter > 20 then
        ui.endRotation(math.sin(event.comboMeter / 180 * 3141.5) * 3 * math.lerpInvSat(event.comboMeter, 20, 30) + 90)
    end
    ui.popFont()
